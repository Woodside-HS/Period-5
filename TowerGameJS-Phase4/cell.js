...
var Cell = function(game, loc, id){
  ...
  this.addNeighbors = function(game, grid){
    this.neighbors = [];    // start with empty neighbors
    var col = this.loc.x/game.w;
    var row = this.loc.y/game.w;
    var n,ne,e,se,s,sw,w,nw = null; // all eight neighbors
  ...
  this.getVector = function(){
    if(this.parent) {
      var dx = this.parent.loc.x - this.loc.x;
      var dy = this.parent.loc.y - this.loc.y;
      var v = new vector2d(dx, dy);
      return v;
    }
